...
| E7  | Mancava la formula delle statistiche per livello; gli `ivs` non comparivano in nessun calcolo.                                                                                            | Definita in Fase 2 come funzione pura `computeStats(...)` in `src/domain/creature/stats.ts`. I tratti sono **modificatori derivati**, mai valori scritti nel salvataggio: ribilanciarne uno non richiede una migrazione.                                                               |
...
### Fase 2 — Combattimento e cattura ✅ completata

Scena Battle con UI in DOM, ATB in modalità wait, 6 tipi in due triangoli, 12 mosse,
5 stati alterati, IA a 3 livelli (casuale / greedy / tattico), cattura con percentuale
visibile prima del lancio, erba alta con spawn pesati per bioma e ora del giorno,
8 specie giocabili, schema di salvataggio 2 con squadra, archivio e inventario.

✅ **1000 combattimenti simulati senza crash** · **durata mediana 25,4 s** (target 20–40) ·
test a seme fisso su danno, cattura, ATB e macchina a stati · **una Comune a HP pieni con
Nodo base si cattura al 30,0%** (target 25–35, con i valori corretti di E1) ·
`balance:sim` fa parte di `npm run verify` e della CI, quindi i criteri non possono
regredire in silenzio.

**Nota sul bilanciamento.** I numeri non sono stati scelti a occhio: `balance-sim`
ha bocciato la prima taratura (mediana 12 s) e la scala degli HP è stata corretta di
